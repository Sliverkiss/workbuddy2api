...
- **隔离号池（内容审核）** — 上游内容审核报错（`moderation_blocked`，「内容未通过安全审核」等）是**账号级**信号：同一段良性内容跨账号可过，说明是被拦的那个账号被上游提权审核了（区别于 `ErrContentBlocked` 的内容指纹误报，后者不罚账号）。内容是**恶性错误**，`quarantine.threshold` 默认 **1**——命中一次即出池进隔离，因为经被标记号的一切请求都可能被拦，用户感知为「这个模型坏了」。隔离与其他状态的本质区别是**静默到期不放行**：`quarantine.probe_delay`（默认 1m，不是惩罚时长）过后只是获得探活资格，须由后台 prober 用良性请求实测——通过（`ReleaseQuarantine`）才回池；仍被拦则 `QuarantineExtend` 按 ×2^hits 续默退避（`quarantine.silence` 起，`quarantine.silence_max` 封顶 24h）；网络 / 限流类无结论不动隔离态，下轮再试。这样被标记的号不会随每次静默到期反复污染号池。隔离号**不进正常选号，也不经全冷却兜底回流**（兜底正是「时间一过又放出来」的漏洞面），`state.json` 落盘对隔离域**不做过期过滤**（到期 ≠ 恢复，重启不得让被标记号直接回池）。隔离与禁用 / 手动停用正交，运维 `revive` 与探活通过同级的放行证据；`/status` 透出 `quarantined` / `quarantine_until` / `quarantine_reason`，隔离号显示 `cool_kind=quarantine`
- **新号观察池** — 新账号首次进池不直接吃主池流量，先过**入池探活**（凭证 / 模型权限可用性），通过后取得「搭车」资格：主池有健康候选时，距上次搭车 ≥ `probation.canary_interval`（默认 5m，**池级单闸**——观察号再多，对主池流量的总侵入也有上界）就把一个既有真实用户请求**改道**给观察号，零新增上游请求；累计 `probation.promote_successes`（默认 3，含探活那次）次成功后毕业，与老号同权。未验活（`passes==0`）的号不进任何真实流量路径——不参与选号、不被 `AvailableUIDs` 收录，连全冷却兜底也显式排除（兜底只按到期时间选号，会绕开健康判定）；主池全空时已验活观察号接单（比 503 强，且不消耗搭车窗口）。入池探活连续无结论达阈进隔离（死凭证号不该被无限探活），被内容审核拦则按恶性错误一次即隔离。选**搭车改道**而非影子测试：影子会让同一出口 IP 的请求量翻倍、直接踩上游 WAF / 风控（WAF 403 修复的教训），且探活只能证明「凭证 + 模型可服务」，证明不了真实负载下的限流 / 计费画像。`/status` 透出 `probation` / `probation_passes`（进度）与汇总计数
