...
//
// 默认 "python3"，与容器/Linux 现状完全一致，行为零变更；WORKBUDDY_PYTHON
// 显式指定时优先，供解释器不叫 python3 的环境使用。
//
...
// 设 WORKBUDDY_PYTHON=python 即可绕过。
func pythonCmd() string {
	if v := strings.TrimSpace(os.Getenv("WORKBUDDY_PYTHON")); v != "" {
		return v
	}
	return "python3"
}
